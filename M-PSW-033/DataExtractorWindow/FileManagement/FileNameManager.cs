...
namespace DataExtractorWindow
{
    ...
    {        
        ...
        public bool Get_FileName_Information(string File_Name, out string Binded_ID, out DateTime File_Date)
        {
            ...
            Date_Time_Stamp[0] = GeneralFunctions.GetHexStringToByteArray(Date[2])[1];
            Date_Time_Stamp[1] = GeneralFunctions.GetHexStringToByteArray(Date[1])[0];
            Date_Time_Stamp[2] = GeneralFunctions.GetHexStringToByteArray(Date[0])[0];
            ...
            Date_Time_Stamp[3] = GeneralFunctions.GetHexStringToByteArray(Time[0])[0];
            Date_Time_Stamp[4] = GeneralFunctions.GetHexStringToByteArray(Time[1])[0];
            Date_Time_Stamp[5] = GeneralFunctions.GetHexStringToByteArray(Time[2])[0];

            // ---- Check if the time stamp read from the file name is valid ----
            DateTime_Status = DateTimeCheck.CheckDateTimeStamp(Date_Time_Stamp, out File_Date);
